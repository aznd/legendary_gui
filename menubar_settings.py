...
def topwindow_settings():
    ...
    label_text = tk.Label(
        ...
        anchor=tk.W, bg=color_light_black,
        fg=color_white)
    label_text.grid(row=1, column=0)
    hyperlink = tk.Label(
        settings_window,
        text="Click here to get your token",
        bg=color_light_black, fg=color_light_blue,
        cursor="hand2", anchor=tk.W)
    hyperlink.grid(row=2, column=0)
    hyperlink.bind(
        "<Button-1>",
        lambda e: callback("https://www.epicgames.com/id/api/redirect"))
    ...
    auth_check = os.path.isfile('~/.config/legendary/user.json')
    if auth_check:
        auth_already_complete = tk.Label(
            settings_window,
            bg=color_light_black,
            text="You are already connected with your Epic Games Account!",
            fg=color_white)
        auth_already_complete.grid(row=5, column=0)
    elif auth_check is False:
        auth_not_complete = tk.Label(
            settings_window,
            text="You are currently not connected to a Epic Games Account!",
            bg=color_light_black, fg=color_white)
...
def auth_button():
    ...
    if(re.search(pattern_auth_token, entry)):
        ...
        auth_not_succesful_try_again = tk.Button(auth_not_succesful,
                                                 text="Try Again",
...
def change_globalrunner_submit_button_func():
    ...
    sql_update_query = f"UPDATE games WHERE gamename = 'globalrunner' SET runner ({data});"
...
def change_globalrunner():
    ...
    submit_button_change_runner = tk.Button(
        topwindow_change_runner, text="Submit",
        command=lambda: change_globalrunner_submit_button_func())
